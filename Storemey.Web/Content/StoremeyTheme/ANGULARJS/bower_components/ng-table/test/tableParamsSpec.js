describe('NgTableParams', function () {
    var scope, NgTableParams, $rootScope;
    ...
    describe('reload', function () {
        ...
        it('should add the results returned by getData to the data field', function () {
            var tp = createNgTableParams({ getData: function () { return [1, 2, 3]; } });
            tp.reload();
            scope.$digest();
        ...
        it('should propagate rejection reason from getData', inject(function ($q) {
            // given
            var tp = createNgTableParams({ getData: function () { return $q.reject('bad response'); } });
            // when
            var actualRejection;
    ...
    describe('getGroups', function () {
        ...
        it('should use group function to group data', function () {
            var grouper = function (item) { return item.name[0]; };
        ...
        it('should use sortDirection defined on group function to sort groups', function () {
            var groupFn = function (item) { return item.role; };
    ...
    describe('getGroups with nested property', function () {
        ...
        it('should use group function to group data', function () {
            var grouper = function (item) { return item.details.name[0]; };
            grouper.sortDirection = '';
            var tp = createNgTableParams({
                count: 2,
                sorting: { 'details.name': 'desc' },
        ...
        it('should use sortDirection defined on group function to sort groups', function () {
            var groupFn = function (item) { return item.details.role; };
            groupFn.sortDirection = 'desc';
            var tp = createNgTableParams({
                count: 3,
                sorting: { 'details.name': 'desc' },
    ...
    describe('isDataReloadRequired', function () {
        ...
        it('should return false when getData fails', inject(function ($q) {
            tp.settings({ getData: function () { return $q.reject('bad response'); } });
            tp.reload();
            scope.$digest();
            expect(tp.isDataReloadRequired()).toBe(false);
        }));
    ...
    describe('hasErrorState', function () {
        ...
        it('should return false once parameter values change', inject(function ($q) {
            // given
            tp = createNgTableParams({ getData: function () { return $q.reject('bad response'); } });
            // when, then
            tp.reload();
            scope.$digest();
            expect(tp.hasErrorState()).toBe(true);
    ...
    describe('interceptors', function () {
        ...
        describe('one response interceptor', function () {
            ...
            it('should be able to modify data returned by getData', function () {
                ...
                var tp = createNgTableParams({ interceptors: [interceptor], getData: function () { return [{}, {}]; } });
                // when
                var actualData;
            ...
            it('should be able to replace data returned by getData', function () {
                // given
                var interceptor = {
                    response: function (data /*, params*/) { return data.map(function (item) { return item * 2; }); }
                };
                var tp = createNgTableParams({ interceptors: [interceptor], getData: function () { return [2, 3]; } });
                // when
                var actualData;
                tp.reload().then(function (data) {
                    actualData = data;
        ...
        describe('one responseError interceptor', function () {
            it('should receive rejections from getData', inject(function ($q) {
                ...
                var tp = createNgTableParams({
                    ...
                    getData: function ( /*params*/) { return $q.reject('BANG!'); }
                });
                // when
                tp.reload();
                scope.$digest();
            ...
            it('should be able to modify reason returned by getData', inject(function ($q) {
                ...
                var tp = createNgTableParams({
                    interceptors: [interceptor],
                    getData: function ( /*params*/) { return $q.reject({ description: 'crappy data' }); }
                });
                // when
                var actualReason;
                tp.reload().catch(function (reason) {
            ...
            it('should be able to replace reason returned by getData', inject(function ($q) {
                // given
                var interceptor = {
                    responseError: function ( /*reason, params*/) { return $q.reject('Cancelled by user'); }
                };
                var tp = createNgTableParams({
                    interceptors: [interceptor],
                    getData: function ( /*params*/) { return $q.reject('BANG!'); }
        ...
        describe('one response plus responseError interceptor', function () {
            it('should NOT call responseError on same interceptor whose response method fails', inject(function ($q) {
                ...
                var interceptor = {
                    hasRun: false,
                    response: function ( /*data, params*/) { return $q.reject('BANG!'); },
                    responseError: function (reason /*, params*/) {
                        this.hasRun = true;
                    }
                };
                var tp = createNgTableParams({
                    interceptors: [interceptor],
                    getData: function ( /*params*/) { return [1, 2, 3]; }
                });
                // when
                tp.reload();
                scope.$digest();
        ...
        describe('multiple response interceptors', function () {
            ...
            it('results of one interceptor should be piped to the next validator', function () {
                ...
                var interceptor2 = {
                    response: function (data /*, params*/) { return data.map(function (item) { return item.toString() + '0'; }); }
                };
                var tp = createNgTableParams({ interceptors: [interceptor, interceptor2], getData: function () { return [2, 3]; } });
                // when
                var actualData;
                tp.reload().then(function (data) {
                    actualData = data;
        ...
        describe('multiple response and responseError interceptors', function () {
            ...
            it('should call next response interceptor when previous interceptor recovers from failure', inject(function ($q) {
                ...
                var recoveringInterceptor = {
                    responseError: function ( /*reason, params*/) { return [8894, 58]; }
                };
                var recoveredData;
                var nextInterceptor = {
                    hasRun: false,
    ...
    describe('events', function () {
        var actualEventArgs, actualPublisher, fakeTableParams, ngTableEventsChannel;
        ...
        function getSubscriberCount() {
            ...
            var ngTableEvents = allEventNames.filter(function (event) { return event.indexOf('ngTable:') === 0; });
        ...
        describe('afterReloadData', function () {
            it('should fire when a reload completes', function () {
                ...
                var params = createNgTableParams({ getData: function () { return newDatapage; } });
                // when
                params.reload();
                scope.$digest();
                // then
            ...
            it('should fire on reload even if datapage remains the same array', function () {
                ...
                var dataPage = [1, 2, 3];
                var params = createNgTableParams({ getData: function () { return dataPage; } });
                // when
                params.reload();
                scope.$digest();
                params.reload();
