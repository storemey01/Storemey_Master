describe('ngTableSelectFilterDs directive', function () {
    ...
    describe('function datasource', function () {
        ...
        it('should keep the array on scope in sync with data array on $column', function () {
            ...
            // when
            var newArray = [{ id: 1, title: 'A' }];
            $scope.$column.data = function () { return newArray; };
            $scope.$digest();
            // then
            expect($scope.$selectData).toBe(newArray);
        });
        ...
        it('should add empty option to a new arriving array', function () {
            ...
            $scope.$digest();
            // when
            $scope.$column.data = function () { return [{ id: 1, title: 'B' }]; };
            $scope.$digest();
            // then
            expect($scope.$selectData).toEqual([{ id: '', title: '' }, { id: 1, title: 'B' }]);
        });
    ...
    describe('asyn function datasource', function () {
        ...
        beforeEach(inject(function (_$timeout_) {
            $timeout = _$timeout_;
            $scope.$column = {
                data: function () { return $timeout(function () { return data; }, 10); }
            };
        }));
        it('should add array to current scope', function () {
            // given
        ...
        it('should keep the array on scope in sync with data array on $column', function () {
            ...
            // when
            var newArray = [{ id: 1, title: 'A' }];
            $scope.$column.data = function () { return $timeout(function () { return newArray; }, 10); };
            $scope.$digest();
            $timeout.flush();
            // then
            expect($scope.$selectData).toBe(newArray);
        ...
        it('should add empty option to a new arriving array', function () {
            ...
            $timeout.flush();
            // when
            $scope.$column.data = function () { return $timeout(function () { return [{ id: 1, title: 'B' }]; }, 10); };
            $scope.$digest();
            $timeout.flush();
            // then
            expect($scope.$selectData).toEqual([{ id: '', title: '' }, { id: 1, title: 'B' }]);
