...
namespace Storemey.Tests
{
    public abstract class StoremeyTestBase : AbpIntegratedTestBase<StoremeyTestModule>
    {
        ...
        protected void UsingDbContext(Action<StoremeyDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<StoremeyDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<StoremeyDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<StoremeyDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<StoremeyDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<StoremeyDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<StoremeyDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<StoremeyDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<StoremeyDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<StoremeyDbContext, Task<T>> func)
        {
            T result;
